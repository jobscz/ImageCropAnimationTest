...
public class FixedCropImageView extends ImageView {
    ...
    public FixedCropImageView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private Rect correctBitmapInViewBound = new Rect();

    public Rect getCorrectBitmapInViewBound() {
        if (mCropPoints == null) {
            return correctBitmapInViewBound;
        }
        Point leftTop = mCropPoints[0];
        Point rightTop = mCropPoints[1];
        Point rightBottom = mCropPoints[2];
        Point leftBottom = mCropPoints[3];

        int correctBitmapWidth = (int) ((getPointsDistance(leftTop, rightTop)
                + getPointsDistance(leftBottom, rightBottom)) / 2);

        int correctBitmapHeight = (int) ((getPointsDistance(leftTop, leftBottom)
                + getPointsDistance(rightTop, rightBottom)) / 2);

        int viewWidth = getWidth();
        int viewHeight = getHeight();

        RectF srcRectF = new RectF(0, 0, correctBitmapWidth, correctBitmapHeight);
        ...
        float scaleX = mMatrixValue[Matrix.MSCALE_X];
        float scaleY = mMatrixValue[Matrix.MSCALE_Y];

        float actWidth = Math.round(correctBitmapWidth * scaleX);
        float actHeight = Math.round(correctBitmapHeight * scaleY);

        float left = (viewWidth - actWidth) / 2;
        float top = (viewHeight - actHeight) / 2;
        float right = left + actWidth;
        float bottom = top + actHeight;

        correctBitmapInViewBound.set((int) left, (int) top, (int) right, (int) bottom);

        return correctBitmapInViewBound;
    }

    public Point[] getCropPointsInView() {

        Point vLeftTop = new Point();
        Point vRightTop = new Point();
        Point vRightBottom = new Point();
        Point vLeftBottom = new Point();

        if (mCropPoints == null) {
            return new Point[]{vLeftTop, vRightTop, vRightBottom, vLeftBottom};
        }
        Point leftTop = mCropPoints[0];
        Point rightTop = mCropPoints[1];
        Point rightBottom = mCropPoints[2];
        ...
        vLeftTop.set((int) getViewPointX(leftTop), (int) getViewPointY(leftTop));
        vRightTop.set((int) getViewPointX(rightTop), (int) getViewPointY(rightTop));
        vRightBottom.set((int) getViewPointX(rightBottom), (int) getViewPointY(rightBottom));
        vLeftBottom.set((int) getViewPointX(leftBottom), (int) getViewPointY(leftBottom));
        return new Point[]{vLeftTop, vRightTop, vRightBottom, vLeftBottom};
    }
